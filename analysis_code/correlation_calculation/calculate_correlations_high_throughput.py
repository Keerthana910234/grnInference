...
#%%
def get_correlations(df, time_pair, genes):
    ...
    for i in range(len(genes)):
        for j in range(i, len(genes)):
            ...
            cross_corr_gene = compute_cross_corr()
            results.append({
                'gene_1': genes[i], 'gene_2': genes[j], 'time': time,
...
#%%
def process_simulation(sim_info):
    """
    ...
    try:
        ...
        corr_all = []
        cross_corr = []
        time_pairs = [(1, 10), (5, 10), (5, 20)]
        for time in time_pairs:
            corr_all.extend(get_correlations_optimized(df, t, genes))
            cross_corr.extend(get_cross_correlations(df, time_points, genes))
        for i, row in enumerate(corr_all):
            row['param_index'] = param_index
            row['cross_corr'] = cross_corr[i]
        del df
        gc.collect()
        return {'correlation': corr_all}
...
def main(path_to_simulations, output_folder, genes, time_points):
    ...
    for i in range(0,end, BATCH_SIZE):
        ...
        results = Parallel(n_jobs=N_JOBS)(
            delayed(process_simulation)((f, path_to_simulations, time_points, genes)) for f in batch
        )
