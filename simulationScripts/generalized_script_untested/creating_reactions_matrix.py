...
def get_new_state(state_array, param_matrix, interaction_matrix, state_var_indices, 
    ...
    # Apply degradation to entire state array slices
...
def create_random_cells(n_genes, num_cells=1000, mean_protein_levels=None):
    ...
    if mean_protein_levels is None:
        ...
        mean_protein_levels = np.array(mean_protein_levels)
    ...
    # Randomly initialize bursting state
    state_array[:, state_var_indices["is_bursting"], :] = np.random.choice([0, 1], size=(n_genes, num_cells))
    
    return state_array, state_var_indices
#%%
def vectorized_reactions_pure_matrix(state_array, param_matrix, interaction_matrix, state_var_indices, 
                                   EC50_matrix=None, global_params=None, t=1/60):
    """
    Pure 3D matrix operations - everything happens simultaneously across all genes and cells.
    ...
    # Short aliases
    s = state_var_indices
    # No need for p = param_matrix.values since we're using column names directly
    
    # Access parameters by column name instead of index
    # param_matrix is a DataFrame, so we can use column names directly
    
    # Global parameters
    splicing_rate = global_params.get("splicing_rate",np.log(2)/(7/60))
    EC50 = global_params.get("EC50", 16)
    n = global_params.get("n", 1)
    ...
    # === STEP 5: K_ON ADJUSTMENT - PURE MATRIX OPERATIONS ===
    ...
    if interaction_matrix is not None and np.any(interaction_matrix != 0):
        ...
        if EC50_matrix is not None:
            ...
            # Hill function for all target-regulator-cell combinations
            # Shape: (n_genes, n_genes, num_cells)
            hill_responses = (protein_3d**n) / (EC50_3d**n + protein_3d**n)
            
            # Apply interaction strengths element-wise
            # interaction_matrix: (n_genes, n_genes) -> (n_genes, n_genes, 1)
            interaction_3d = interaction_matrix[:, :, np.newaxis]
            weighted_responses = hill_responses * interaction_3d
            
            # Sum regulatory effects for each target gene across all regulators
            # Sum along regulator axis (axis=1): (n_genes, n_genes, num_cells) -> (n_genes, num_cells)
            regulatory_effects = np.sum(weighted_responses, axis=0)
            
        else:
            # Fallback to uniform EC50
            EC50 = global_params.get("EC50", 16)
            protein_3d = protein_levels[np.newaxis, :, :]  # (1, n_genes, num_cells)
            hill_responses = (protein_3d**n) / (EC50**n + protein_3d**n)  # (1, n_genes, num_cells)
            
            # Apply interaction matrix: (n_genes, n_genes) @ (n_genes, num_cells)
            regulatory_effects = np.einsum('ij,jk->ik', interaction_matrix, hill_responses.squeeze(0))
...
#%%
#Test zone for each function
path_to_matrix = "/home/mzo5929/Keerthana/grnInference/simulationData/general_simulation_data/test_data/matrix101.txt"
n_genes, matrix = read_input_matrix(path_to_matrix)
parameter_sheet_path = "/home/mzo5929/Keerthana/grnInference/simulationData/parameter_sweep.csv"
param_matrix, row_mapping = assign_parameters_to_genes(parameter_sheet_path, rows=[0,3], n_random=n_genes)
# gene_params = params_to_gene_structure(param_dict, n_genes=n_genes)
EC_50_matrix = create_EC50_matrix(param_matrix, matrix)
state_array, state_var_indices = create_random_cells(n_genes, num_cells=1000, mean_protein_levels = EC_50_matrix)

#First do burn in

#Then actual simulation
new_state = get_new_state(state_array, param_matrix, interaction_matrix, state_var_indices, 
                                   EC50_matrix, global_params, t=1/60)
