...
class Cell:
    ...
    def runSimulation(self, time, showPlot = False, baseSeed = None, speciesIndex = None, updatePropensity = None, parameterUpdateValues = None, logQueue = None, maintain = False, speciesToPerturb = None, timeOfPerturbation = None, amountToPerturb = None):
        ...
        #Identifying indices to perturb - gene state vs mRNA
        if speciesToPerturb != None:
            # print(speciesToPerturb)
            if isinstance(speciesToPerturb, str):
                speciesToPerturb = [speciesToPerturb]

            speciesPerturbed = [key for key in speciesIndexToProvide if any(sub in key for sub in speciesToPerturb)]
            # print(speciesPerturbed)
            reactionIndexToPerturb = np.array([speciesIndexToProvide[key] for key in speciesIndexToProvide if any(species in key for species in speciesPerturbed) and "mRNA" in key])
            # print(speciesIndexToPerturb)
        else:
            reactionIndexToPerturb = None
        ...
        newSamples = gillespieSSA(
            ...
            baseSeed=baseSeed, 
            logQueue = logQueue,
            reactionIndexToPerturb = reactionIndexToPerturb,
